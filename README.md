...
- Flags:
  ...
  - `--ir-out=PATH`: write the generated IR to the given path, creating parent directories if needed. Without this flag, IR is not written to a file.
- Exit codes:
...
- IR is generated on every successful compilation; it is printed to stdout only when `ir` debug is enabled. Use `--ir-out` to save it to disk.

...
//TODO:Check all commas
  ...
  private fun parseFunctionItem(): FunctionItemNode {
    ...
    val selfParam =
      if (peek(1)?.type == Keyword.SELF || peek(2)?.type == Keyword.SELF || peek(3)?.type == Keyword.SELF) {
    ...
    while (peek(1)?.type != Punctuation.RIGHT_PAREN) {
      ...
      if (peek(1)?.type != Punctuation.RIGHT_PAREN) {
        expectAndConsume(Punctuation.COMMA)
      } else tryConsume(Punctuation.COMMA)
    }
  ...
  private fun parseImplItem(): ImplItemNode {
    ...
    val id = if (peek(2)?.type == Keyword.FOR) {
  ...
  private fun parsePrefix(): ExprNode {
    return when (peek(1)?.type) {
      ...
      Identifier, Keyword.SELF, Keyword.SELF_UPPER -> parsePathExpr()
...
//            Keyword.MATCH -> parseMatchExpr()
  ...
  private fun parseBorrowExpr(): BorrowExprNode {
    val isDouble = tryConsume(Punctuation.AND_AND)
    if (!isDouble) expectAndConsume(Punctuation.AMPERSAND)
    val isMut = tryConsume(Keyword.MUT)
    return if (isDouble) BorrowExprNode(BorrowExprNode(parseExpr(), isMut), isMut) else BorrowExprNode(
      parseExpr(190),
      isMut
    )
  }

  private fun parseDerefExpr(): DerefExprNode {
    expectAndConsume(Punctuation.STAR)
    return DerefExprNode(parseExpr(190))
  }
  ...
  private fun parseWhileExpr(): WhileExprNode {
    ...
    val cond = parseConds()
    expectAndConsume(Punctuation.RIGHT_PAREN)
    return WhileExprNode(cond, parseBlockExpr())
  }
    ...
    if (!tryConsume(Punctuation.RIGHT_BRACKET)) {
      ...
      when (peek(1)?.type) {
        Punctuation.SEMICOLON -> {
          ...
          return ArrayExprNode(listOf(), first, second)
        }
...
//    }
  ...
  private fun parseConds(): List<CondExprNode> {
    ...
    while (peek(1)?.type != Punctuation.RIGHT_PAREN) {
      ...
      if (peek(1)?.type != Punctuation.RIGHT_PAREN) expectAndConsume(Punctuation.AND_AND)
    }
    return conds
  }

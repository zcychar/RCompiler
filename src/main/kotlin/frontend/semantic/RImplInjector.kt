...
class RImplInjector(val gScope: Scope, val crate: CrateNode) : ASTVisitor<Unit> {
        ...
        when (target) {
            is Enum -> {
                ...
                target.associateItems.put(function.name, function)
                    ?: throw CompileError("Semantic: duplicated associate item for struct $target, name $function")
            ...
            is Struct -> {
                ...
                function.self = typedSelf
                target.associateItems.put(function.name, function)
                    ?: throw CompileError("Semantic: duplicated associate item for enum $target, name $function")
                if (function.selfParam != null) {
                    target.methods.put(function.name, function)
                }
            }
                ...
                throw CompileError("Semantic: impl target type ${target} not found")
            }
        }
    }
    ...
    fun injectConstImpl(target: Symbol, const: Constant) {
        when (target) {
            is Enum, is Struct -> {
                target.associateItems.put(const.name, const)
                    ?: throw CompileError("Semantic: duplicated associate item for struct/enum $target, name $const")
            }
    ...
    fun checkTraitConstant(const: Constant, trait: Trait) {
        val constInTrait = trait.type.associatedItems[const.name] as? Constant
            ?: throw CompileError("Semantic: implementing a trait with wrong constant $const")
        if (constInTrait.type != const.type) {
            throw CompileError("Semantic: implementing a trait with wrong constant $const, expect $constInTrait")
        }
    }
    ...
    fun checkTraitFunction(function: Function, trait: Trait) {
        val functionInTrait = trait.type.associatedItems[function.name] as? Function
            ?: throw CompileError("Semantic: implementing a trait with wrong function $function")
        ...
        if (!isSame) {
            throw CompileError("Semantic: implementing a trait with wrong function $function, expect $functionInTrait")
        }
    }
    ...
    override fun visit(node: ImplItemNode) {
        ...
        val symbol = if (node.type is TypePathNode && node.type.name != null) {
            currentScope?.resolve(node.type.name, Namespace.TYPE)
                ?: throw CompileError("Semantic: impl target type ${node.type} not found")
            ...
            when (it) {
                is ConstItemNode -> {
                    val const = currentScope?.resolve(it.name, Namespace.VALUE) as? Constant
                        ?: throw CompileError("Semantic: impl associate does not found in $node")
                ...
                else -> throw CompileError("Semantic: invalid impl $node")
            }
        }
        trait?.type?.associatedItems?.forEach {
            when (it.value) {
                is Constant -> {
                    if ((it.value as Constant).value == null) {
                        currentScope?.resolve(it.value.name, Namespace.VALUE) as? Constant
                            ?: throw CompileError("Semantic: impl for trait ${trait.name}, associate ${it.value} does not found in $node")
                    }
                }
                ...
                else -> throw CompileError("Semantic: invalid associate item ${it.value} of trait")
            }
        }
        currentScope = currentScope?.parentScope()
    }

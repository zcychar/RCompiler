...
class RSymbolResolver(val gScope: Scope, val crate: CrateNode) : ASTVisitor<Unit> {
    ...
    fun evaluateConstExpr(expr: ExprNode, expectType: Type): ConstValue {
        return when (expr) {
            ...
            is CastExprNode -> {
                val innerExpr = evaluateConstExpr(expr.expr, ErrorType)
                if (expr.targetType is TypePathNode && expr.targetType.name != null) {
                    val exprType = getTypeFromConst(innerExpr)
                    val targetSymbol = currentScope?.resolve(expr.targetType.name, Namespace.TYPE)
                    when (targetSymbol) {
                        is BuiltIn -> {
                            val targetType = targetSymbol.type
                            return if (isInt(exprType) && isInt(targetType)) {
                                ConstValue.Int((innerExpr as ConstValue.Int).value, targetType)
